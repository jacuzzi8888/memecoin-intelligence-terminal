...
import { AlertTriangle, BellRing, CheckCircle2, Clock3, ExternalLink, Filter, Radio } from "lucide-react";
import {
  ErrorState,
  LoadingRows,
  MetricCard,
...
export default function AlertsPage() {
  ...
  useEffect(() => {
    async function fetchAlerts() {
      ...
      try {
        ...
        const data: { success?: boolean; data?: AlertData[]; error?: string } = await alertsRes.json();
  ...
  async function reviewAlert(verdict: "valid" | "false_positive" | "uncertain") {
    ...
    try {
      ...
      const payload: {
        success?: boolean;
        error?: string;
        data?: { verdict: "valid" | "false_positive" | "uncertain"; notes?: string | null; reviewedAt: string };
      } = await res.json();
      if (!payload.success || !payload.data) throw new Error(payload.error ?? "Review could not be saved.");
      setAlerts((current) => current.map((alert) => alert.id === selectedAlert.id
        ? { ...alert, review: { verdict, notes: payload.data?.notes ?? null, reviewedAt: payload.data?.reviewedAt ?? new Date().toISOString() } }
        : alert));
      setReviewNote("");
      setOutcomeMessage(`Review saved: ${verdict.replace("_", " ")}.`);
    } catch (reviewError) {
      setOutcomeMessage(reviewError instanceof Error ? reviewError.message : "Review could not be saved.");
    } finally {
      setReviewing(false);
    }
  }
  ...
  const staleCount = alerts.filter((alert) => Date.now() - new Date(alert.dataFreshness).getTime() > 20 * 60 * 1000).length;
  ...
  return (
    <div className="space-y-4">
      {error ? <ErrorState title="Alerts degraded" message={error} /> : null}

      <div className="grid gap-4 md:grid-cols-5">
        <MetricCard label="Active Alerts" value={alerts.length} tone="primary" />
        <MetricCard label="Critical" value={criticalCount} tone={criticalCount ? "danger" : "default"} />
        <MetricCard label="Needs Review" value={needsReviewCount} tone={needsReviewCount ? "warning" : "success"} />
        <MetricCard label="15m Avg" value={formatPct(fifteenMinuteSummary?.avgReturnPct)} tone={(fifteenMinuteSummary?.avgReturnPct ?? 0) >= 0 ? "success" : "danger"} />
        <MetricCard label="1h Win Rate" value={oneHourSummary ? `${Math.round(oneHourSummary.winRate * 100)}%` : "n/a"} />
      </div>

      <div className="rounded-lg border border-outline bg-surface-container px-standard py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge tone={staleCount ? "stale" : "success"}>{staleCount} stale source(s)</StatusBadge>
            <StatusBadge tone="primary">{filteredAlerts.length} filtered</StatusBadge>
            {outcomeSummary.slice(0, 5).map((item) => (
              <StatusBadge key={item.outcomeType} tone={item.avgReturnPct >= 0 ? "success" : "danger"}>
                {outcomeLabel(item.outcomeType)} {formatPct(item.avgReturnPct)}
              </StatusBadge>
            ))}
            {outcomeMessage ? <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-on-surface-variant">{outcomeMessage}</span> : null}
          </div>
          <button
        ...
        <Panel
          ...
          action={
            <div className="flex gap-2">
              {["all", "needs_review", "critical", "high", "medium"].map((item) => (
                <button
                  key={item}
                  onClick={() => setFilter(item)}
                  className={`rounded-sm border px-2 py-1 font-mono text-[11px] uppercase tracking-[0.12em] ${
                    filter === item
        ...
        >
          <div className="divide-y divide-outline">
            {filteredAlerts.length === 0 ? (
              <div className="p-standard text-sm text-on-surface-variant">No alerts match the active filter.</div>
            ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <StatusBadge tone={alert.priority === "critical" ? "danger" : alert.priority === "high" ? "warning" : "primary"}>
                            {alert.priority}
                          </StatusBadge>
                          <p className="truncate font-semibold text-on-surface">{alert.title}</p>
                        </div>
                        <p className="mt-2 line-clamp-2 text-sm text-on-surface-variant">{alert.message ?? alert.status}</p>
                        <div className="mt-3 flex flex-wrap items-center gap-3 font-mono text-[11px] uppercase tracking-[0.12em] text-on-surface-variant">
                          ...
                          <StatusBadge tone={alert.review?.verdict === "false_positive" ? "danger" : alert.review ? "success" : "warning"}>
                            {alert.review?.verdict?.replace("_", " ") ?? "needs review"}
                          </StatusBadge>
                          {alert.outcomes.slice(0, 2).map((outcome) => (
                            <span key={`${alert.id}-${outcome.outcomeType}`} className={(outcome.outcomeValue ?? 0) >= 0 ? "text-success" : "text-destructive"}>
                      ...
                      <div className="text-right">
                        <p className={`font-mono text-lg ${scoreTone(alert.signalScore)}`}>{alert.signalScore}</p>
                        <p className="font-mono text-[11px] uppercase tracking-[0.12em] text-on-surface-variant">{alert.status}</p>
                      </div>
                    </div>
                  </button>
                );
          ...
          {selectedAlert ? (
            <div className="space-y-4 p-standard">
              <div className="rounded-lg border border-outline bg-surface px-4 py-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <StatusBadge tone={selectedAlert.priority === "critical" ? "danger" : selectedAlert.priority === "high" ? "warning" : "primary"}>
                      {selectedAlert.priority}
                    </StatusBadge>
                    <h2 className="mt-3 text-xl font-semibold text-on-surface">{selectedAlert.title}</h2>
                  </div>
                  <AlertTriangle className={selectedAlert.priority === "critical" ? "h-5 w-5 text-destructive" : "h-5 w-5 text-warning"} />
                </div>
                <p className="mt-3 text-sm text-on-surface-variant">{selectedAlert.message ?? "No message payload stored."}</p>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <MetricCard label="Score" value={selectedAlert.signalScore} tone={selectedAlert.signalScore >= 80 ? "success" : "warning"} />
                ...
                {selectedAlert.outcomes.length > 0 ? (
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {selectedAlert.outcomes.map((outcome) => (
                      <div key={outcome.outcomeType} className="rounded-sm border border-outline bg-surface-container px-3 py-3">
                        <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-on-surface-variant">{outcomeLabel(outcome.outcomeType)}</p>
                        <p className={`mt-2 font-mono text-lg ${(outcome.outcomeValue ?? 0) >= 0 ? "text-success" : "text-destructive"}`}>
                  ...
                  <p className="mt-3 text-on-surface-variant">No measured outcomes yet. Backfill after enough market snapshots exist.</p>
                )}
              </div>
              ...
              <div className="rounded-lg border border-warning/30 bg-warning/5 px-4 py-4 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-on-surface-variant">
                      {selectedAlert.reviewRecommendation?.reason ?? "Review the alert against its measured outcome."}
                    </p>
                  </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button onClick={() => reviewAlert("valid")} disabled={reviewing} className="rounded-sm border border-success/40 bg-success/10 px-3 py-2 font-mono text-[11px] uppercase tracking-[0.12em] text-success disabled:opacity-50">Accept signal</button>
                  <button onClick={() => reviewAlert("false_positive")} disabled={reviewing} className="rounded-sm border border-destructive/40 bg-destructive/10 px-3 py-2 font-mono text-[11px] uppercase tracking-[0.12em] text-destructive disabled:opacity-50">False positive</button>
                  <button onClick={() => reviewAlert("uncertain")} disabled={reviewing} className="rounded-sm border border-outline bg-surface-container px-3 py-2 font-mono text-[11px] uppercase tracking-[0.12em] text-on-surface-variant disabled:opacity-50">Keep uncertain</button>
                </div>
                {selectedAlert.review?.notes ? <p className="mt-3 text-xs text-on-surface-variant">Previous note: {selectedAlert.review.notes}</p> : null}
              </div>
              ...
              <div className="rounded-lg border border-outline bg-surface px-4 py-4 text-sm">
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex justify-between gap-4">
                    ...
                    <span className="font-mono text-on-surface">{selectedAlert.strategyName ?? "Unknown"}</span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span className="text-on-surface-variant">Token</span>
                    <span className="font-mono text-primary">{shortAddress(selectedAlert.tokenAddress, 8, 6)}</span>
                  </div>
                  <div className="flex justify-between gap-4">
              ...
              <div className="grid gap-2">
                ...
                <button className="flex items-center justify-center gap-2 rounded-sm border border-outline bg-surface px-4 py-3 font-mono text-[11px] uppercase tracking-[0.12em] text-on-surface">
                  <CheckCircle2 className="h-4 w-4 text-success" />
                  Acknowledge
                </button>
              </div>

              <div className="rounded-sm border border-stale/40 bg-stale/10 px-3 py-3 text-sm">
                <div className="flex gap-2">
                  <Clock3 className="mt-0.5 h-4 w-4 shrink-0 text-stale" />
                  <p className="text-on-surface-variant">Suppression and snooze controls should remain confirmation-gated before activation.</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="p-standard text-sm text-on-surface-variant">Select an alert to inspect its routing and research context.</div>
          )}
        </Panel>
      </div>
    </div>
