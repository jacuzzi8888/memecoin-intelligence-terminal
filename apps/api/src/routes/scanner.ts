...
import { createTokenDiscoveryRepository, discoverTokens, discoverWalletsFromRecentTokens } from "@memecoin/indexer";
...
const querySchema = z.object({
  ...
  dataSource: z.enum(["all", "dexscreener", "helius", "birdeye", "solana-rpc", "development"]).default("all"),
  discoverySource: z.enum(["all", "dexscreener-profile", "dexscreener-boost-latest", "dexscreener-boost-top", "helius", "rpc"]).default("all"),
  priority: z.enum(["critical", "high", "medium", "low", "info"]).optional(),
  search: z.string().trim().min(1).max(80).optional(),
  sinceDays: z.coerce.number().min(1).max(30).default(1),
});

const liveScanBodySchema = z.object({
...
const walletDiscoveryBodySchema = z.object({
...
function buildSnapshotExistsFilter(
  since: Date,
...
export const scannerRoute: FastifyPluginAsync = async (app) => {
  app.get("/scanner", async (request, reply) => {
    ...
    const orderCol = query.sortBy === "signal_score" ? schema.signals.signalScore :
                    ...
                    query.sortBy === "pair_age" ? pairCreatedAtExpr :
                    schema.signals.detectedAt;
    const orderFn = query.sortBy === "pair_age"
    ...
    if (query.minLiquidityUsd !== undefined) {
      conditions.push(buildSnapshotExistsFilter(since, sql`latest_filter_snapshot.liquidity_usd >= ${String(query.minLiquidityUsd)}`));
    }
    if (query.maxLiquidityUsd !== undefined) {
      conditions.push(buildSnapshotExistsFilter(since, sql`latest_filter_snapshot.liquidity_usd <= ${String(query.maxLiquidityUsd)}`));
    }
    if (query.minVolume1hUsd !== undefined) {
      conditions.push(buildSnapshotExistsFilter(since, sql`latest_filter_snapshot.volume_1h_usd >= ${String(query.minVolume1hUsd)}`));
    }
    if (query.minVolume24hUsd !== undefined) {
      conditions.push(buildSnapshotExistsFilter(since, sql`latest_filter_snapshot.volume_24h_usd >= ${String(query.minVolume24hUsd)}`));
    }
    if (query.minMarketCapUsd !== undefined) {
      conditions.push(buildSnapshotExistsFilter(since, sql`latest_filter_snapshot.market_cap_usd >= ${String(query.minMarketCapUsd)}`));
    }
    if (query.maxMarketCapUsd !== undefined) {
      conditions.push(buildSnapshotExistsFilter(since, sql`latest_filter_snapshot.market_cap_usd <= ${String(query.maxMarketCapUsd)}`));
    }
    ...
    const snapshotByToken = new Map<string, {
      snapshotAt: Date;
    ...
    const tradeRows = tokenAddresses.length > 0
      ? await db.select().from(schema.walletTrades)
        .where(and(inArray(schema.walletTrades.tokenAddress, tokenAddresses), gte(schema.walletTrades.tradedAt, since)))
        .orderBy(desc(schema.walletTrades.tradedAt))
        .limit(500)
      : [];
    const tradeWalletIds = [...new Set(tradeRows.map((trade) => trade.walletId))];
    const [tradeWalletRows, performanceRows] = tradeWalletIds.length > 0
      ? await Promise.all([
        db.select().from(schema.wallets).where(inArray(schema.wallets.id, tradeWalletIds)),
        db.select().from(schema.walletPerformance).where(inArray(schema.walletPerformance.walletId, tradeWalletIds)),
      ])
      : [[], []];
    ...
    const evidenceByToken = new Map<string, {
      tradeCount: number;
    ...
    return {
      ...
      data: results.map((r) => {
        ...
        holderEvidence: {
          ...
          sampledHolders: typeof holderEvidence.sampledHolders === "number" ? holderEvidence.sampledHolders : 0,
          topHolderConcentrationPct: typeof holderEvidence.topHolderConcentrationPct === "number"
        ...
        pair: (() => {
          ...
          return {
            ...
            pairAgeMinutes: pairCreatedAt ? Math.max(0, Math.floor((Date.now() - pairCreatedAt.getTime()) / 60_000)) : null,
          };
        })(),
  ...
  app.post("/scanner/live-scan", { config: { rateLimit: { max: 4, timeWindow: "1 minute" } } }, async (request) => {
  ...
  app.post("/scanner/discover-wallets", { config: { rateLimit: { max: 2, timeWindow: "1 minute" } } }, async (request) => {
